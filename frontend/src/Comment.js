...
class Comment extends Component {
    ...
    componentDidMount() {
        LyricsAPI.getPostDetails(this.props.match.params.postid).then((post) => {    
          this.props.dispatch(getPostDetails({ 
              post 
            }))
        })
        LyricsAPI.getComments(this.props.match.params.postid).then((comments) => {    
    ...
    handleSubmit = (e) => {
        ...
        LyricsAPI.submitComment(values).then(comment => {
    ...
    handleVoteComment = (commentid, option) =>  {
        ...
        LyricsAPI.voteComment(commentid, option).then (() => {
    ...
    handleVotePost = (postid, option) =>  {
        var increment = (option == "upVote") ? 1 : -1
        LyricsAPI.votePost(postid, option).then (() => {
